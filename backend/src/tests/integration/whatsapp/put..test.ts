...
describe("PUT /whatsapp", () => {
  ...
  it("should update a whatsapp successfully", async () => {
    const response = await request("http://localhost:3000")
      .put(`/whatsapp/${whatsapp.id}`)
      .send({
        farewellMessage: "teste update",
        greetingMessage: "teste update",
        isDefault: false,
        name: "teste update",
        queueIds: [],
      })
    ...
    console.log("should update a whatsapp successfully", response.body);
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty("id");
    expect(response.body.farewellMessage).toBe("teste update");
    expect(response.body.greetingMessage).toBe("teste update");
    expect(response.body.isDefault).toBe(false);
    expect(response.body.name).toBe("teste update");
    expect(response.body.queues).toStrictEqual([]);
  });

  it("should update a whatsapp with queue successfully", async () => {
    const response = await request("http://localhost:3000")
      .put(`/whatsapp/${whatsapp.id}`)
      .send({
        farewellMessage: "teste",
    ...
    console.log(
      "should update a whatsapp with queue successfully",
      response.body
    );
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty("id");
  ...
  it("should not update a whatsapp with two queues sem greetingMessage", async () => {
    const response = await request("http://localhost:3000")
      .put(`/whatsapp/${whatsapp.id}`)
      .send({
        farewellMessage: "teste",
    ...
    console.log(
      "should not update a whatsapp with two queues sem greetingMessage",
      response.body
    );
    expect(response.status).toBe(400);
  });

  it("should not update whatsapp with duplicate name", async () => {
    ...
    const response = await request("http://localhost:3000")
      ...
      .set("Authorization", `Bearer ${token}`);
    console.log(
      "should not update whatsapp with duplicate name",
      response.body
    );
    expect(response.status).toBe(400);
